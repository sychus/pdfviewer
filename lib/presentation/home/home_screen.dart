...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _open(DocumentSource source, {bool alreadyBusy = false}) async {
    ...
    try {
      // Fingerprint: 128KB of raw file bytes. No pdfrx, no PDF parsing.
      // Near-instant even for gigabyte files.
      final file = File(source.uri);
      final docId = await widget.identity.compute(FileDocumentBytes(file));

      // Restore saved position. PageCount is unknown here (PdfViewer
      // will report it via onDocumentLoaded), so we can't clamp.
      // A past-end page is harmless — PdfViewer opens at the last page.
      ...
      // Navigate. PdfViewer.file() is the ONLY thing that opens the PDF.
      // No PdfDocument.openFile() anywhere else in this flow.
